...
def test_get_question_method_add_value_to_question_to_analyse_attribute(user_question, monkeypatch):
...
@pytest.mark.parametrize("user_question", [
    "Bonjour GrandPy, j'espère que tu vas bien ! Connais-tu le lieu suivant : Place de la Bastille ? Bien à toi.",
    "BoNjoUr granDpy, j'Espère que tU vAs bien ! connais-tu LE lieu suivant : place de la BasTille ? bien à toi.",
    "BONJOUR GRANDPY, J'ESPèRE QUE TU VAS BIEN ! CONNAIS-TU LE LIEU SUIVANT : PLACE DE LA BASTILLE ? BIEN à TOI."    
])
...
def test_stopwords_list_attribute_is_assigned_well_to_attribute(monkeypatch):
    ...
    stopwords = ["a", "un", "une", "as", "des", "le", "la", "d", "et", "de", "s","il","te","plait"]
...
@pytest.mark.parametrize("question", [
    ("""salut grandpy! Comment s est passe ta soiree avec grandma hier soir? au fait, pendant que j y pense,
    pourrais tu m indiquer ou se trouve le musee d art et d histoire de fribourg s il te plait?""", "le musee d art et d histoire de fribourg s il te plait"),
    ("""bonsoir grandpy, j espere que tu as passe une belle semaine. est ce que tu pourrais m'indiquer
       l adresse de la tour eiffel? merci d avance et salutations a mamie.
    """, "de la tour eiffel")
])
...
@pytest.mark.parametrize("question", [
    ("""Salut grandpy! Comment s'est passé ta soirée avec Grandma hier soir? Au fait, pendant que j'y pense,
    pourrais-tu m'indiquer où se trouve le musée d'art et d'histoire de Fribourg, s'il te plaît?""", "musee art histoire fribourg"),
    ("""
